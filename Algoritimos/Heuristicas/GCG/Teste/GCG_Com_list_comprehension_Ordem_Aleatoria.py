...
for e in range(20000):
    ...
    coluna = 0
    acabou = False
    while len(lista_poligonos)>0 or not acabou:
        colocou = False
        ...
        if not colocou:
            #tempoF+=exact_fit(-1,int(0),int(base),0,False,0.83,base,altura)
            acabou = True
            with open('Inferencia_GCG.csv', 'a', newline='') as arquivo:
                writer = csv.writer(arquivo)
                writer.writerow([e,(round(((sum(lista_area_fechos)+AreaPreenchida)/(340*380))*100,2)),len(CordenadasProibidas), (tempoF/60)])
            break  
        ...
        if colocou:
            ...
            if coluna == 2:
                tag = True
            res,tem=exact_fit(coluna,int(adicional),int(adicional2),0,tag,0.83,base,altura)
            tempoF=tem
            if res is not None:
                adicional+=res
            coluna+=1
