...
def segmentos_se_cruzam(p1, p2, q1, q2):
    ...
    # Casos especiais para bordas colineares
    if o1 == 0 and na_segmento(p1, p2, q1):
        return False  # Compartilham a borda
    ...
    return False  # Não se cruzam ou compartilham bordas
...
def gerar_coluna(n,adicional,draw):
    ...
    for j in range(0, len(lista_poligonos), 1):
        acabou_coluna = False
        for grau in graus:
            for i in range(adicional, base, Passo):
...
def Greedy_Column_Generation(Base=None,Altura=None,x=-200,y=200,peças='fu',draw=False,escala=None,passo = 1):
...
Greedy_Column_Generation(peças='fu')
