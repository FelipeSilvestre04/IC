...
def gerar_coluna(n,adicional):
    ...
    for j in range(0, len(lista_poligonos), 1):
        ...
        for grau in graus:
            for i in range(adicional, base, 5):
                ...
                on_click2((-200) + i, -180, grau, fecho_turtle, dados_turtle, Bot, j, 3, base, altura)
                if tamanho > len(lista_poligonos):
                    adicional2 = variacao_total_eixo_x(fecho)
                    exact_fit(coluna, int(adicional), int(adicional2), 0, False, 0.83, base, altura, False)
                    acabou_coluna = True
                    colunas.append([j, (AreaPreenchida/(380*adicional2))*(len(CordenadasProibidas)), len(CordenadasProibidas)])
    ...
    for f in range(adicional, base, 5):
        ...
        on_click2((-200)+f, -180, grau, fecho_turtle, dados_turtle, Bot, melhor_coluna[0], 3, base, altura)
        if tamanho > len(lista_poligonos):
            break
    adicional2 = variacao_total_eixo_x(fecho)
    adicional1,tem = exact_fit(n, int(adicional), int(adicional2), 0, False, 0.83, base, altura, True)
    Area.CordenadasProibidas[0][0]= -200
    Area.CordenadasProibidas[1][0]= 140
    Area.CordenadasProibidas[2][0]= 140
    Area.CordenadasProibidas[3][0]= -200
...
adicional = 0
print(adicional)
adicional += gerar_coluna(0,int(adicional))
print(adicional)
adicional += gerar_coluna(1,int(adicional))
print(adicional)
adicional += gerar_coluna(2,int(adicional))
print(adicional)
