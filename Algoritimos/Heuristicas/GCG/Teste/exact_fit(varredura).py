...
def exact_fit(n):
    ...
    colocou = False
    graus = [0,90,180,270]
    on_click2(-199, -184,0,fecho_turtle,dados_turtle,Bot,n)
    while len(lista_poligonos) > 0:
        possiveis_posições = []
                        ...
                        while j < 385:
                            ...
                            if cordenada_disponivel:
                                ...
                                if (len(lista_poligonos)>0):
                                    if cordenada_disponivel:
                                        #draw_piece(x_transformado,y_transformado,0)
                                        area_desperdiçada = on_click2(x_transformado, y_transformado,grau,fecho_turtle,dados_turtle,Bot,l)
                                if len(lista_poligonos)<tamanho:
                                    colocou = True
                                    porcentagem_ocupamento = (AreaPreenchida/(340*380))*100
                                    on_click2(400,200,grau,fecho_turtle,dados_turtle,Bot,l)
        ...
        if (len(possiveis_posições)>0):
            for k in range(len(possiveis_posições)):
                if possiveis_posições[k][1] > menor:
                    ...
                    index = k
            on_click2(possiveis_posições[index][2],possiveis_posições[index][3],possiveis_posições[index][4],fecho_turtle,dados_turtle,Bot,possiveis_posições[index][0])
...
for k in range(10,len(lista_poligonos),1):
    exact_fit(k)
    print(k, (AreaPreenchida/(340*380))*100,len(CordenadasProibidas))
    turtle.clearscreen()
    lista_poligonos = ler_poligonos('fu.dat')
    lista_poligonos.pop(0)
    Bot = Botao(350, 300, 200, 150)
