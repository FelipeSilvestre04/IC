...
class envCSP(gym.Env):
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-1, high=42, shape=(len(self.ambiente.lista), 16), dtype=np.int8)                   
    ...
    def reset(self, seed=None):
        ...
        self.pecas = np.array(polygon_features(copy.deepcopy(self.pecas_locsearch), self.pecas))
        self.matriz = np.array(self.ambiente._matriz, dtype=np.int8)

        self.estado = self.pecas
    ...
    def step(self, action):
        ...
        teste_lista[idx1],teste_lista[idx2] = teste_lista[idx2],teste_lista[idx1]
        ...
        for peca in teste_lista[1:]:
            if peca in self.ambiente.lista:
                index1 = self.ambiente.lista.index(peca)
                peca_posicao = PackS(self.ambiente, index1, self.rotacoes, False,self.tabela_nfps)
        ...
        if not self.done:
            if self.melhorou_global < 5:
                ...
                print(f"T: {round(self.end_time - self.start_time,2)}s | F:{round(self.porcentagem * 100, 2)}% , {self.pecas_posicionadas}/12 --- I:{self.pinicial * 100}% , {self.pecas_iniciais}/12 ||| R:{[res for res in self.lista_recompensa if res > 0]}, SR:{round(sum(self.lista_recompensa),2)} | NA {self.acao}")
        else:
            self.end_time = time.time()
            print(f"T: {round(self.end_time - self.start_time,2)}s | F:{round(self.porcentagem * 100, 2)}% , {self.pecas_posicionadas}/12 --- I:{self.pinicial * 100}% , {self.pecas_iniciais}/12 ||| R:{[res for res in self.lista_recompensa if res > 0]}, SR:{round(sum(self.lista_recompensa),2)} | NA {self.acao}")
...
def polygon_features(lista_poligonos,cordenadas_poligonos):
    features = []
    i = 0
    for coords in lista_poligonos:
        ...
        features.append([
            ...
            width, 
            height,
            cordenadas_poligonos[i][0][0],
            cordenadas_poligonos[i][0][1],
            cordenadas_poligonos[i][1][0],
            cordenadas_poligonos[i][1][1],
            cordenadas_poligonos[i][2][0],
            cordenadas_poligonos[i][2][1],
            cordenadas_poligonos[i][3][0],
            cordenadas_poligonos[i][3][1],
        ])
        i+=1
...
if __name__ == '__main__':
    ...
    model = PPO(
       "MlpPolicy",  # Política para entradas múltiplas
        envs,
        verbose=1,
        tensorboard_log="log_dir",  # Substitua pelo caminho desejado
        n_steps=30,               # Maior número de passos para capturar mais transições
        batch_size=120,             # Tamanho de lote ajustado para estabilidade
        n_epochs=15,                # Redução para evitar overfitting
        gamma=0.99,                 # Considerar recompensas de longo prazo
        learning_rate=linear_schedule(4e-4, 1e-4)  # Taxa de aprendizado adaptativa
