...
class envCSP(gym.Env):
    ...
    def reset(self, seed=None):
        ...
        self.melhor_porcentagem = round(self.ambiente.area_ocupada/self.ambiente.area, 2)

        self.melhor_fecho = round(self.ambiente.area_ocupada/self.ambiente.fecho, 2)
    ...
    def step(self, action):
        ...
        melhorou = area_nova > self.melhor_porcentagem
        
        if melhorou:
            self.pecas_locsearch = copy.deepcopy(teste_lista)
            teste_lista = []
            self.recompensa = (area_nova - self.melhor_porcentagem)*100
            self.porcentagem = area_nova
            self.melhor_porcentagem = area_nova
            self.melhor_fecho = fecho_novo
         ...
         #   self.pecas_locsearch = copy.deepcopy(teste_lista)
          #  teste_lista = []
           # self.recompensa = fecho_novo - self.melhor_fecho
            #self.porcentagem = area_nova
            #self.fecho = fecho_novo
            #self.melhor_porcentagem = area_nova
            #self.melhor_fecho = fecho_novo
            ...
            self.pecas_locsearch = copy.deepcopy(teste_lista)
            teste_lista = []
            self.recompensa = (area_nova - self.melhor_porcentagem)
            self.porcentagem = area_nova
            self.fecho = fecho_novo
            self.pecas_posicionadas = len(self.ambiente.pecas_posicionadas)
        ...
        if not self.done:
            if self.melhorou_global < 15:
                self.done = False
                #print(self.acao)
            else:
                #print(self.acao)
                self.done = True
                print(f"F:{round(self.porcentagem * 100, 2)}% , {self.pecas_posicionadas}/12 --- I:{self.pinicial * 100}% , {self.pecas_iniciais}/12, --- M:{self.melhor_porcentagem} ||| R:{[res for res in self.lista_recompensa if res > 0]}, SR:{round(sum(self.lista_recompensa),2)} | NA {self.acao}")
...
if __name__ == '__main__':
    ...
    model = PPO('MultiInputPolicy', envs, verbose=1, tensorboard_log=log_dir,
                n_steps=600,  # Ajustado para ser múltiplo de batch_size
                batch_size=120,  # Aumentado para melhor estabilidade
                n_epochs=30,  # Reduzido para treinamento mais rápido
