...
if __name__ == '__main__':
    ...
    model = DQN(
        ...
        buffer_size=500000,               # Buffer maior
        batch_size=64,                    # Lote maior para estabilidade
        learning_rate=linear_schedule(5e-4, 1e-4),  # Taxa de aprendizado adaptativa
        train_freq=4,                     # Atualização a cada 4 passos
        gamma=0.99,                       # Considerar recompensas de longo prazo
        exploration_fraction=0.2,         # Exploração inicial prolongada
        exploration_final_eps=0.05,       # Exploração mínima final
        target_update_interval=500,       # Atualizações frequentes da rede-alvo
        policy_kwargs=dict(net_arch=[256, 256])  # Rede maior para capturar padrões complexos
    )
