...
class envCSP(gym.Env):
    ...
    def step(self, action):
        ...
        if random.randint(0,99) < 5:
            action = random.randint(0,len(self.ambiente.lista)-1)
        ...
        if action < len(self.ambiente.lista):
            if self.acao == 1:
                ...
                area = (self.ambiente.area_ocupada/self.ambiente.area)
                ...
                if peca_posicao is not False:
                    ...
                    if len(self.ambiente.pecas_posicionadas) == self.ambiente.max_pecas:
                        ...
                        self.recompensa = (self.ambiente.area_ocupada/self.ambiente.fecho)
...
# Definir os parâmetros da política
    policy_kwargs = dict(
        net_arch=[128, 128, 128]  # Três camadas com 128 unidades cada
    )

    # Criar o modelo PPO com a nova arquitetura de rede
    model = PPO("MultiInputPolicy", env, verbose=1, tensorboard_log=log_dir, policy_kwargs=policy_kwargs,ent_coef=0.01)
