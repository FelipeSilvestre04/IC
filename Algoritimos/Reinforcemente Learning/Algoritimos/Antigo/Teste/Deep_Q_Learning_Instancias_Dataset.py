...
class env2KP(gym.Env):
    ...
    def verificar_final(self, n):
        ...
        if not lista_poligonos:
            ...
            area_total = self.base*self.altura
            preenchimento  = areaPreenchida/area_total
            desperdicio = 1 - (areaPreenchida/self.fecho)
            tamanho_fecho = 1 - (self.fecho/area_total)
            recompensa = preenchimento - desperdicio - tamanho_fecho
...
model = PPO('MultiInputPolicy', env, verbose=1,tensorboard_log=log_dir,n_steps=1200, batch_size=200,n_epochs=20)
