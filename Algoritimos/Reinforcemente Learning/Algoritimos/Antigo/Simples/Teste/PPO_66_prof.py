...
class env2KP(gym.Env):
    ...
    def calcular_recompensa_intermediaria(self, areaPreenchida, num_pecas_colocadas, total_pecas):
        ...
        #return preenchimento_total - (1 - eficiencia_fecho)
        #return (area/self.area) - (1-eficiencia_fecho)
        #return preenchimento_total
        return self.calcular_recompensa()
    ...
    def calcular_recompensa(self):
        ...
        if self.fecho>0:
            desperdicio_fecho = 1 - (float(preenchimento)/float(self.fecho))
        else:
            desperdicio_fecho = 0
        if n >0:  
            return pt - (desperdicio_fecho*n)
        else:
            return -1
