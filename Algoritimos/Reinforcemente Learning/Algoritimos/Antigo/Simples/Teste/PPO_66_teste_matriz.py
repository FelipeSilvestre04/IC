    ...
    def _atualizar_matriz(self):
        ...
        for poligono in self._poligonos:
            poligono_transformado = [
                transformar_coordenadas(x, y, self.x, self.x + self._base, self.y - self._altura, self.y, self._base,
                                        self._altura) for x, y in poligono]
...
def criar_matrizes_poligonos(vertices_lista, n, W, H):
    # Determina o tamanho da matriz com base no maior polígono
    max_width = W
    max_height = H

    matrizes_poligonos = []
    for vertices in vertices_lista:
        matriz = Matriz(max_width, max_height, 0, max_height)
        matriz._criar_poligono(vertices)
        matriz._plot()
        matrizes_poligonos.append(matriz._matriz)
        #matriz.fechar_janela()

    return matrizes_poligonos
...
class env2KP(gym.Env):
    def __init__(self, Base=None,Altura=None,x=-200,y=200,peças='fu',draw=False,escala=None,plot=False,passo=1,peso1_coluna = 2, peso2_coluna = 1, peso3_coluna = 2, peso1_pecas = 0, peso2_pecas = 1, verificacao_avancada = False):
        ...
        self.max_width = max(max(x for x, y in poligono) for poligono in nova_lista)
        self.max_height = max(max(y for x, y in poligono) for poligono in nova_lista)
        self.pieces_completo = (criar_matrizes_poligonos(nova_lista, self.numero_pecas, self.max_width, self.max_height))
        ...
        self.observation_space = spaces.Dict({
            ...
            'pieces': spaces.Box(low=0, high=1, shape=(len(lista_poligonos), self.max_height, self.max_width), dtype=np.int8)  # Usando apenas um polígono por vez
        })
    ...
    def reset(self, seed=None):
        ...
        lista_poligonos = ler_poligonos(instancias)
        nova_lista, nova_lista_completa = tratar_lista(lista_poligonos)
        self.pieces = self.pieces_completo

        combinada = list(zip(lista_poligonos, nova_lista, nova_lista_completa, self.pieces))
        random.shuffle(combinada)
        lista_poligonos, nova_lista, nova_lista_completa, self.pieces = zip(*combinada)

        lista_poligonos = list(lista_poligonos)
        nova_lista = list(nova_lista)
        nova_lista_completa = list(nova_lista_completa)
        self.pieces = list(self.pieces)
        
        cutting_area = np.array((Matriz_np._matriz), dtype=np.int8)

        pieces = np.array((self.pieces), dtype=np.int8)
    ...
    def preencher_pecas(self):
        while len(self.pieces) < self.numero_pecas:
            self.pieces.append(np.zeros((self.max_height, self.max_width), dtype=np.int8))

        return np.array((self.pieces), dtype=np.int8)
    ...
    def verificar_final(self, n):
        ...
        if not lista_poligonos:
            self.done = True
            print(f"{round((areaPreenchida / (self.base * self.altura)) * 100, 2)}%, {len(CordenadasProibidas)}/12")
            ...
            if self.acao > 1:
                recompensa = ((areaPreenchida/(self.base*self.altura)))  - (1-((areaPreenchida)/(self.fecho)))
                if (1-((areaPreenchida)/(self.fecho))) < 0.1:
                    recompensa = 1
            else:
...
def linear_schedule(initial_value):
    def func(progress):
        # Progresso vai de 1 (início) a 0 (fim)
...
if __name__ == '__main__':
    ...
    model = PPO('MultiInputPolicy', envs, verbose=1, tensorboard_log=log_dir, n_steps=100, batch_size=400, n_epochs=20, gamma=0.9, policy_kwargs=policy_kwargs, learning_rate=linear_schedule(5e-4), ent_coef=0.01)
