...
class env2KP(gym.Env):
    def __init__(self, Base=None,Altura=None,x=-200,y=200,peças='fu',draw=False,escala=None,plot=False,passo=1,peso1_coluna = 2, peso2_coluna = 1, peso3_coluna = 2, peso1_pecas = 0, peso2_pecas = 1, verificacao_avancada = False):
        ...
        self.observation_space = spaces.Dict({
            ...
            'vetor': spaces.Box(low=-1, high=255, shape=(1, 5), dtype=np.int8)
        })
    ...
    def reset(self, seed=None):
        ...
        vetor = [self.base, self.altura, 0, 0, 12]
        self.state = {
    ...
    def atualizar_estado(self, pontos_pol):
        ...
        vetor = [self.base, self.altura, areaPreenchida/self.area, (1-(areaPreenchida/self.fecho)), self.numero_pecas - (len(CordenadasProibidas))]
        self.state = {'cutting_area': cutting_area, 'pieces': pieces, 'vetor':vetor}
    ...
    def verificar_final(self, n):
        ...
        if not lista_poligonos:
            ...
            if self.fr:
                if self.fecho > 0:
                    recompensa = self.calcular_recompensa_intermediaria(areaPreenchida, len(CordenadasProibidas), self.numero_pecas)
                else:
                    recompensa =-1
            else:
                recompensa = -1
            #recompensa -= 0.2*(self.numero_pecas - len(CordenadasProibidas))/12
            ...
            print(f"{round((areaPreenchida / (self.base * self.altura)) * 100, 2)}%, {len(CordenadasProibidas)}/12, R:{round(media,2)}, FR:{self.dist}")
            self.dist = 0
            self.fr = False
            ...
            elif len(CordenadasProibidas) > 1:
                if self.fr:
                    recompensa = self.calcular_recompensa_intermediaria(areaPreenchida, len(CordenadasProibidas), self.numero_pecas)
                else:
                    recompensa = -1
    ...
    def calcular_recompensa_intermediaria(self, areaPreenchida, num_pecas_colocadas, total_pecas):
        ...
        #return max(preenchimento_total - (1 - eficiencia_fecho) - (1 - preenchimento_total), -1)
...
if __name__ == '__main__':
    ...
    model = PPO('MultiInputPolicy', envs, verbose=1, tensorboard_log=log_dir,
                n_steps=100,  # Ajustado para ser múltiplo de batch_size
                batch_size=400,  # Aumentado para melhor estabilidade
                n_epochs=5,  # Reduzido para treinamento mais rápido
                gamma=0.99,  # Aumentado para considerar recompensas de longo prazo
                learning_rate=linear_schedule(5e-4, 0),
                ent_coef=0.01,
                clip_range=linear_schedule(0.4, 0.1))
