        ...
        if n == 0:
            if self.acao == 1:
                ...
                if self.fr:
                    ...
                    epsilon = 1e-2  # Defina uma tolerância pequena
                    if abs(maior - area_peca) < epsilon:
                        recompensa = min(areaPreenchida/self.fecho,1)
                    else:
                        recompensa = area_peca - maior
                else:
                    recompensa = -1        
...
if __name__ == '__main__':
    ...
    model = PPO('MultiInputPolicy', envs, verbose=1, tensorboard_log=log_dir,
                ...
                batch_size=60,  # Aumentado para melhor estabilidade
                n_epochs=10,  # Reduzido para treinamento mais rápido
                gamma=0.999,  # Aumentado para considerar recompensas de longo prazo
