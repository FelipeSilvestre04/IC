...
class env2KP(gym.Env):
    def __init__(self, Base=None,Altura=None,x=-200,y=200,peças='fu',draw=False,escala=None,plot=False,passo=1,peso1_coluna = 2, peso2_coluna = 1, peso3_coluna = 2, peso1_pecas = 0, peso2_pecas = 1, verificacao_avancada = False):
        ...
        self.action_space = spaces.MultiDiscrete([self.base,self.altura,len(lista_poligonos), 360])
    ...
    def step(self, action):
        ...
        if n < len(lista_poligonos):
            posição_disponivel, pontos_pol = self.verificar_posicao_disponivel(n, x, y, grau)
            if posição_disponivel:
                self.remover_peca(n)
...
if __name__ == '__main__':
    ...
    model = PPO('MultiInputPolicy', envs, verbose=1, tensorboard_log=log_dir, n_steps=800, batch_size=400, n_epochs=20, gamma=0.9, policy_kwargs=policy_kwargs, learning_rate=3e-4, ent_coef=0.01)
