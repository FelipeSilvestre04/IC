...
class env2KP(gym.Env):
    def __init__(self, Base=None,Altura=None,x=-200,y=200,peças='fu',draw=False,escala=None,plot=False,passo=1,peso1_coluna = 2, peso2_coluna = 1, peso3_coluna = 2, peso1_pecas = 0, peso2_pecas = 1, verificacao_avancada = False):
        ...
        self.observation_space = spaces.Dict({
            ...
            'pieces': spaces.Box(low=-1, high=255, shape=(len(lista_poligonos), 8), dtype=np.int8)
        })
    ...
    def verificar_final(self, n,peca):
        ...
        if n == 0:
            if self.acao == 1:
                area_peca = calcular_area(CordenadasProibidas[-1])/self.area
                nova_lista.append(CordenadasProibidas[-1])
                maior = max((calcular_area(peca)/self.area) for peca in nova_lista)
                nova_lista.remove(CordenadasProibidas[-1])
                epsilon = 1e-2  # Defina uma tolerância pequena
                if abs(maior - area_peca) < epsilon:
                    pol = self.primeiro_poligono[-1]
                    self.primeiro_poligono.pop
                    min_dist = 10000
                    for cor in pol:
                ...
                if self.fr:
                    ...
                    epsilon = 1e-2  # Defina uma tolerância pequena
                    if abs(maior - area_peca) < epsilon:
                        fecho_retangular = area_fecho_retangular(CordenadasProibidas)
                        recompensa = min((areaPreenchida/fecho_retangular)**1, 1)

                    else:
                       #recompensa = -1
                       recompensa = max(((area_peca - maior)*1), -1 )
            ...
            if self.fr:
                ...
                #recompensa = round(-(1 * (area/area_total)),2)
...
def area_fecho_retangular(poligonos):
    # Inicializando com valores extremos opostos
    min_x = float('inf')
    max_x = float('-inf')
    min_y = float('inf')
    max_y = float('-inf')

    # Iterando por todos os polígonos e seus vértices
    for poligono in poligonos:
        for vertice in poligono:
            x, y = vertice
            # Atualizando os valores mínimos e máximos
            min_x = min(min_x, x)
            max_x = max(max_x, x)
            min_y = min(min_y, y)
            max_y = max(max_y, y)
    
    # Calculando largura e altura do fecho retangular
    largura = max_x - min_x
    altura = max_y - min_y

    # Calculando e retornando a área
...
class RewardCallback(BaseCallback):
    ...
    def _on_step(self):
        # Captura a recompensa do último passo
        reward = self.locals['rewards'][0]
        self.rewards.append(reward)
        
        # A cada 1000 passos, imprime a recompensa média
        if self.n_calls % 1000 == 0:
            avg_reward = sum(self.rewards[-1000:]) / len(self.rewards[-1000:])
            print(f"Passo {self.n_calls}, Recompensa média: {avg_reward:.2f}")
...
if __name__ == '__main__':
    ...
    model = PPO('MultiInputPolicy', envs, verbose=1, tensorboard_log=log_dir,
                ...
                gamma=0.7,  # Aumentado para considerar recompensas de longo prazo
