...
class env2KP(gym.Env):
    ...
    def verificar_final(self, n):
        ...
        if not lista_poligonos:
            ...
            recompensa = ((areaPreenchida/(self.base*self.altura)))  - (2*(1-((areaPreenchida)/(self.fecho))))
            return self.state, float(recompensa), self.done, False, {}
        elif n ==1:
...
if __name__ == '__main__':
    ...
    model = PPO('MultiInputPolicy', envs, verbose=1, tensorboard_log=log_dir, n_steps=1200, batch_size=400, n_epochs=20, gamma=0.9, policy_kwargs=policy_kwargs, learning_rate=3e-4, ent_coef=0.01)
