...
class env2KP(gym.Env):
    def __init__(self, Base=None,Altura=None,x=-200,y=200,peças='fu',draw=False,escala=None,plot=False,passo=1,peso1_coluna = 2, peso2_coluna = 1, peso3_coluna = 2, peso1_pecas = 0, peso2_pecas = 1, verificacao_avancada = False):
        ...
        self.action_space = spaces.MultiDiscrete([self.base,self.altura,len(lista_poligonos)])
    ...
    def reset(self, seed=None):
        ...
        max_vertices = self.observation_space['pieces'].shape[1]
        ...
        if self.fecho>0:
            preenchimento_fecho = np.int8((float(areaPreenchida)/float(self.fecho))*100)
        else:
            preenchimento_fecho = np.int8(0)
            vetor_caracteristicas = np.array([[np.int8((areaPreenchida/(self.base*self.altura))*100)], [preenchimento_fecho], [np.int8(len(lista_poligonos))]], dtype=np.int8)
            self.state = {
    ...
    def step(self, action):
        ...
        if n < len(lista_poligonos):
            posição_disponivel, pontos_pol = self.verificar_posicao_disponivel(n, x, y)
            if posição_disponivel:
                self.remover_peca(n)
    ...
    def verificar_posicao_disponivel(self, n, x, y):
        pol = nova_lista_completa[n]
        x1, y1 = transformar_coordenadas_inversa(x, y, self.X, self.X + self.base, self.Y, self.Y - self.altura, self.base, self.altura)
        pontos_pol = [(int(x1) + int(rotate_point(cor[0], cor[1], 0)[0]), int(y1) + int(rotate_point(cor[0], cor[1], 0)[1])) for cor in pol]
    ...
    def verificar_final(self, n):
        ...
        if not lista_poligonos:
            ...
            recompensa = (1-((self.fecho-areaPreenchida)/(self.base*self.altura)))
    ...
    def calcular_recompensa(self):
        ...
        if n >0:  
            return float((preenchimento/(self.altura*self.base)))*(len(CordenadasProibidas))
        else:
            return float(0)
...
model = A2C('MultiInputPolicy', env, verbose=1,tensorboard_log=log_dir,n_steps=1200)
