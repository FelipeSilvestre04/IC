...
class envCSP(gym.Env):
    ...
    def reset(self, seed=None):
        ...
        if (self.epoca - 1) % 10 == 0:
            self.I1,self.I2 = IniSOL(self.ambiente, self.rotacoes,0,False,self.tabela_nfps)
            I1 = copy.deepcopy(self.I1)
            I2 = copy.deepcopy(self.I2)

            
            self.pinicial_0 = round(self.ambiente.area_ocupada/self.ambiente.area, 2)
            self.porcentagem_0 = round(self.ambiente.area_ocupada/self.ambiente.area, 2)
            self.fecho_0 = round(self.ambiente.area_ocupada/self.ambiente.fecho, 2)
            ...
            self.pinicial = self.pinicial_0
            self.porcentagem = self.porcentagem_0
            self.fecho = self.fecho_0
    ...
    def step(self, action):
        ...
        if melhorou:
            ...
            self.pecas_locsearch = copy.deepcopy(teste_lista)
            teste_lista = []
            self.recompensa = -1 * (1 - fecho_novo)
            self.porcentagem = area_nova
            self.fecho = fecho_novo
            self.pecas_posicionadas = len(self.ambiente.pecas_posicionadas)
            self.melhorou_global = 0               
            ...
            if idx1 == idx2:
                ...
                self.recompensa = (area_nova - self.porcentagem) - 1
        ...
        if self.done:
            ...
            if (self.epoca % 100) == 0:
                print(f"--- ULTIMOS MAXIMOS: {self.maximos} ---")
                self.maximos = 0
...
if __name__ == '__main__':
    ...
    model = PPO(
        ...
        batch_size=60,             # Tamanho de lote ajustado para estabilidade
