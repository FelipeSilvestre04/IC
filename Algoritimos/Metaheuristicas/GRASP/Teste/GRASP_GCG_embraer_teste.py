...
def GRASP_GCG(mIter, x,q1,q2,q3,rotacoes,pecas_inisol, instancia,tabela_nfps,base=None, altura=None):
    print("\nGRASP GCG iniciado!")
    for i in range(mIter):
        print(f'Iteração {i}/{mIter} GRASP GCG.')
        ambiente = CSP(dataset=instancia, render=False, plot=False,Base=base,Altura=altura) 
        I1_ini, I2_ini,pecasI = IniSOL(ambiente,rotacoes,pecas_inisol, False, tabela_nfps,Base=base,Altura=altura)
        
       
        print(porcentagem_area(pecasI,ambiente))
        if len(I1_ini) == ambiente.max_pecas:
            return I1_ini,pecasI
        fecho = ambiente.area_ocupada/ambiente.fecho
        I1_novo,pecas_novo = LocSEARCH(I1_ini,I2_ini,x,q1,q2,q3,fecho, 0,instancia,tabela_nfps,Base=base,Altura=altura,solucao = pecasI)
        print(porcentagem_area(pecas_novo,env) * 100)
        print((sum([calcular_area(pol) for pol in pecas_novo])/ambiente.area))
        if len(I1_novo) == ambiente.max_pecas:
            return I1_novo,pecas_novo
        
        if i == 0:
            I_f = copy.deepcopy(I1_novo)
            pecas = copy.deepcopy(pecas_novo)

        elif porcentagem_area(pecas_novo,ambiente) > porcentagem_area(pecas,ambiente):
            I_f = copy.deepcopy(I1_novo)
            pecas = pecas_novo
...
def LocSEARCH(I1, I2, x_dado, q1, q2, q3,fecho, pq,instancia,tabela_nfps,Base,Altura,solucao):
    melhorou_global = True
    
    while melhorou_global:
        ambiente = CSP(dataset=instancia,render=False, plot=False,Base=Base,Altura=Altura)        
        melhorou_global = False
        area_global = porcentagem_area(solucao,ambiente)
        for idk in range(0,x_dado,1):
            ...
            for peca in I1_linha[1:]:
                if peca[-1] in ambiente.lista:
                    ...
                    peca_posicao = PackS_GG(ambiente, index, rotacoes, False,tabela_nfps)
            ...
            area_nova = porcentagem_area(ambiente.pecas_posicionadas,ambiente)
            ...
            if len(I1) == ambiente.max_pecas:
                ...
                return I1,ambiente.pecas_posicionadas 
...
def IniSOL(ambiente, rotacoes, pq, draw, tabela_nfps, Base, Altura):
    I1_inisol = []  # Peças posicionadas
    I2_inisol = []  # Peças não posicionadas
    ...
    while not acabou:
        ...
        for peca in indexs:
            peca_posicao = PackS_GG(ambiente, peca, rotacoes,draw,tabela_nfps)
            if peca_posicao is not False:
        ...
        if Possiveis_posicoes:
            ...
            I1_inisol.append(melhor_posicao)
            if draw:
                for i in range(1000):
            ...
            if ambiente.lista:
                for peca in ambiente.lista:
                    I2_inisol.append(peca)
            acabou = True

    print("Peças posicionadas:", len(I1_inisol))
    print("Peças restantes:", len(I2_inisol))
    print(porcentagem_area(ambiente.pecas_posicionadas,ambiente) * 100)
   
    return I1_inisol, I2_inisol, ambiente.pecas_posicionadas  # Retorna a lista de peças posicionadas e as restantes
...
def porcentagem_area(I1,ambiente):
    ...
    porcentagem = round((area/ambiente.area)*100,2)
    return porcentagem
...
def PackS_GG(ambiente, peca, rotacoes,draw,tabela_nfps):
    nfp = []
    lista = []
    melhores_posicoes = []
...
def ExSEARCH(ambiente, peca, grau_indice, draw, tabela_nfps):
    ...
    # Calcula NFPs como na função original
    nfps = []
    ...
    nfp = list(combinar_poligonos(nfps).exterior.coords) if nfps else []

    # Lógica de varredura adaptada
    ...
    # Varredura do espaço
    for i in range(0, ambiente.base, Passo):
        for j in range(0, ambiente.altura, Passo):
            x_t, y_t = transformar_coordenadas_inversa(i, j, 
                                                     ambiente.x, 
                                                     ambiente.x + ambiente.base, 
                                                     ambiente.y, 
                                                     ambiente.y - ambiente.altura, 
                                                     ambiente.base, 
                                                     ambiente.altura)
            
            if draw:
                for k in range(10):
                    ambiente.draw_click(x_t, -y_t, grau_indice, peca, False)
            ...
            posicao_valida = True
            for x1, y1 in pontos:
                if y1 > ambiente.y or not ponto_dentro_poligono(x1, y1, ambiente.cordenadas_area, True):
                    ...
                    #for inter in interpolacoes:
                        ...
                        #for Q1,Q2,Q3 in pesos:
                            #for vin in vizinhancas:   
                                ...
                                Stime = time.time()
                                tabela_nfps = pre_processar_NFP(rotacoes, env.nova_lista,0)
                                Etime = time.time()
                                with open(filename, "a") as file:
                                    file.write("---------------------------------------------------------------------------------------------\n")
                                ...
                                for i in range(5):
                                    ...
                                    pecas,solucao = GRASP_GCG(max_iter, vizinhanca, q1, q2, q3, rotacoes,int(round(len(env.lista)*0.0,0)),instancia, tabela_nfps,400,400)
                                    end_time = time.time()  # Termina a medição do tempo
                                    execution_time = end_time - start_time  # Calcula o tempo de execução
                                    #env = CSP(instancia,render=False,plot=False)
                                    print(f"Iteracao {i+1}: {len(pecas)} pecas, {sum([calcular_area(pol) for pol in solucao])/env.area} , {execution_time:.2f} segundos")

                                    resultado = {
                                        "iteration": i + 1,
                                        "num_pieces": len(pecas),
                                        "area_percent":  (sum([calcular_area(pol) for pol in solucao])/(400*400)) * 100,
