...
cordenadas_poligono2 = []
poligonos_pulados2 = []
def exact_fit(geraçao, matriz,cordenadas_poligono,poligonos_pulados):
    poligonos_pulados = []
    t = turtle.Turtle()
    turtle.tracer(0)
    t.color("black")
    ...
    while len(Gerador._geracao) > 0:
        ...
        while i < base:
            ...
            while j < altura:
                ...
                if Gerador.Colocou:
                    cordenadas_poligono.append((x_transformado,y_transformado))
                    if len(Gerador._geracao)>0:
                ...
                if Gerador.Colocou:
                    cordenadas_poligono.append((x2_transformado,y2_transformado))
                    if len(Gerador._geracao)>0:
                        i, j = int(float(Gerador._geracao[-1].split(',')[3])), int(float(Gerador._geracao[-1].split(',')[3]))+1
                        i2, j2 = 600-int(float(Gerador._geracao[-1].split(',')[3])), 400-int(float(Gerador._geracao[-1].split(',')[3]))+1
                        break
                if len(Gerador._geracao)>0:
                    if int(Gerador._geracao[-1].split(',')[0])==4:
                        j+=1
                        j2-=1
                    else:
                        j+=10
            ...
            if len(Gerador._geracao)>0:        
                ...
                i2 -= 10
        if len(Gerador._geracao)>0:
            poligonos_pulados.append(Gerador._geracao[-1])
            Gerador._geracao.pop()

    return poligonos_pulados,cordenadas_poligono
...
def busca_local_grasp(cordenadas_poligono, poligonos_pulados, maior_porcentagem, matriz):
    # Copia a lista de coordenadas para não modificar a original
    solucao = cordenadas_poligono.copy()
        # Escolhe dois polígonos aleatoriamente
    i, j = random.sample(range(len(solucao)), 2)

        # Troca as posições dos dois polígonos
    solucao[i], solucao[j] = solucao[j], solucao[i]
    zeros_coords = np.argwhere(matriz == 0)

# Escolha aleatoriamente uma coordenada com valor zero
    for i in range(len(poligonos_pulados)*2, 0, -1):
    # Seu código aqui
        coordenada_escolhida = zeros_coords[np.random.randint(len(zeros_coords))]
        i,j = coordenada_escolhida[0],coordenada_escolhida[1]
        x1,y1 = transformar_coordenadas_inversa(i, j, x, 250, y, -100, base, altura)
        solucao.append((x,y))

        # Avalia a nova solução (aqui você deve substituir por sua própria função de avaliação)

    for par in solucao:
        on_click2(par[0],par[1])
        if not Gerador.Colocou:
            solucao.remove(par)
            Gerador._geracao.pop()


    for pol in poligonos_pulados:
        Gerador._geracao.append(pol)
    
    Gerador._geracao = sorted(Gerador._geracao, key=lambda x: (float(x.split(',')[0]=='4'), float(x.split(',')[1])), reverse=False)
    
    exact_fit(Gerador._geracao, np.array((matriz2._matriz)),solucao,poligonos_pulados)
    porcentagem_atual = round(Area.AreaOcupada/(base*altura)*100, 1)

    if  porcentagem_atual > maior_porcentagem:
        cordenadas_poligono = solucao.copy()  # Atualiza a melhor solução encontrada

    return cordenadas_poligono
...
turtle.tracer(1)
ordem_inicial = Gerador._geracao.copy()
ordem_turtles = Gerador._turtles.copy()
with open('Heuristica Normal Dupla.csv', 'a', newline='') as arquivo:
        writer = csv.writer(arquivo)
        writer.writerow(['Heuristica Varredura Dupla + GRASP - Mistas:'])
...
if int(tempo_execucao) != int(tempo_anterior):
    tempo(tempo_execucao)
    tempo_anterior = int(tempo_execucao)
poligonos_pulados2,cordenadas_poligono2 = exact_fit(Gerador._geracao, np.array((matriz2._matriz)),cordenadas_poligono2,poligonos_pulados2)
with open('Heuristica Normal Dupla.csv', 'a', newline='') as arquivo:
    writer = csv.writer(arquivo)
    writer.writerow(['exact fit',(round(Area.AreaOcupada/(base*altura)*100, 1)),len(ordem_inicial) - len(poligonos_pulados2) ])
maior_porcentagem = (round(Area.AreaOcupada/(base*altura)*100, 1))
print(cordenadas_poligono2)
for i in range(100):
    Area.Last_Reward = []
    Area.AreaTotal = []
    ...
    Gerador._count = 0
    Gerador._geracaopop = []
    Gerador._geracao = ordem_inicial.copy()
    Gerador._turtles = ordem_turtles.copy()
    ...
    cordenadas_poligono = busca_local_grasp(cordenadas_poligono2, poligonos_pulados2, maior_porcentagem, np.array(matriz2._matriz))
    with open('Heuristica Normal Dupla.csv', 'a', newline='') as arquivo:
        writer = csv.writer(arquivo)
        writer.writerow([i+1, (round(Area.AreaOcupada/(base*altura)*100, 1)),len(cordenadas_poligono)])
