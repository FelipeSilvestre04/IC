    ...
    while melhorou_global:        
        ...
        for idk in range(0,x_dado,1):
            ...
            if len(I1) == 12:
                ...
                print(ambiente.pecas_posicionadas)
                if False:
                    turtle.clear()
                ...
                return I1,np.sum(ambiente._matriz)/ambiente._matriz.size
    ...
    for x,y in nfp:
        #for flip in [True, False]:
                ...
                if posicao_valida:
                    ...
                    possiveis_posicoes.append([peca,x,y,grau_indice,flip,round(fecho_retangular_max,2),ambiente.lista[peca]])
...
for j in range(3):
    ...
    for i in range(10):
        ...
        pecas,area = GRASP(max_iter, vizinhanca, q1, q2, q3, rotacoes, j)
        end_time = time.time()  # Termina a medição do tempo
        execution_time = end_time - start_time  # Calcula o tempo de execução
        print(f"Iteracao {i+1}: {len(pecas)} pecas, {porcentagem_area(pecas)} x {area*100} , {execution_time:.2f} segundos")
