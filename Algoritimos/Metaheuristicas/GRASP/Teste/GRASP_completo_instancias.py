...
def GRASP(mIter, x,q1,q2,q3,rotacoes,pecas_inisol, instancia):
...
def LocSEARCH(I1, I2, x_dado, q1, q2, q3,fecho, pq,instancia):
...
for instancia in instancias:
    with open(filename, "a") as file:
        file.write("---------------------------------------------------------------------------------------------\n")
        file.write(f"GRASP ORIGINAL: iteracoes maxima {max_iter}, {vizinhanca} vizinhancas, {12} pecas inisol, q1,q2,q2 = {q1},{q2},{q3}\n")

    tempos = []

    for i in range(1000):
        start_time = time.time()  # Inicia a medição do tempo
        pecas = GRASP(max_iter, vizinhanca, q1, q2, q3, rotacoes, 0, instancia)
