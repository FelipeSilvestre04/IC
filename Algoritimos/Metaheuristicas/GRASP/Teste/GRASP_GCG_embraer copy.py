...
def GRASP_GCG(mIter, x,q1,q2,q3,rotacoes,pecas_inisol, instancia,tabela_nfps,base=None, altura=None):
    ...
    for i in range(mIter):
        ...
        I1_novo,pecas_novo = LocSEARCH(I1_ini,I2_ini,x,q1,q2,q3,fecho, 0,instancia,tabela_nfps,Base=base,Altura=altura)
        ...
        elif porcentagem_area(I1_novo,ambiente) > porcentagem_area(I_f,ambiente):
            I_f = copy.deepcopy(I1_novo)
            pecas = copy.deepcopy(pecas_novo)
...
def LocSEARCH(I1, I2, x_dado, q1, q2, q3,fecho, pq,instancia,tabela_nfps,Base,Altura):
...
def IniSOL(ambiente, rotacoes, pq, draw, tabela_nfps, Base, Altura):
    ...
    print(round((sum([calcular_area(pol) for pol in ambiente.pecas_posicionadas])/ambiente.area))*100,2)
...
def ExSEARCH(ambiente, peca, grau_indice, draw, tabela_nfps):
    ...
    # Varredura do espaço
    for i in range(comeco_base, ambiente.base, Passo):
            ...
            #for vin in vizinhancas:
                    ...
                    #for inter in interpolacoes:
                        ...
                        #for Q1,Q2,Q3 in pesos:
                            #for vin in vizinhancas:   
                                env = CSP(instancia,render=False,plot=False,Base=400, Altura=400)
                                Stime = time.time()
                                ...
                                for i in range(5):
                                    ...
                                    print(f"Iteracao {i+1}: {len(pecas)} pecas, {round(sum([calcular_area(pol) for pol in solucao])/(400*400)) * 100,2} , {execution_time:.2f} segundos")

                                    resultado = {
                                        "iteration": i + 1,
                                        "num_pieces": len(pecas),
                                        "area_percent":  (round(sum([calcular_area(pol) for pol in solucao])/(400*400)) * 100,2),
