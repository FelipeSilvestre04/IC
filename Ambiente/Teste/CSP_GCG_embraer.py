...
class CSP():
    def __init__(self,dataset='fu',Base=None,Altura=None,Escala=None,render=False,plot=True, x=-200, y=200, epslon = 1):
        ...
        epsilon = epslon  # Parâmetro de suavização para simplificação de vértices
...
def suavizar_poligono(index, poligonos, iteracoes=1, tamanho_kernel=5, area_minima=50, epsilon=5.0):
    ...
    print(f"Polígono original: {len(poligono)} vértices")
    for i, novo_poly in enumerate(novos_poligonos):
        print(f"Polígono suavizado {i + 1}: {len(novo_poly)} vértices")
