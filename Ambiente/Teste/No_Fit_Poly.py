...
def on_click(x,y):
    on_click2(x,y,grau,fecho_turtle,dados_turtle,Bot)



def Busca_Local(lista):

    i, j = random.sample(range(len(lista)), 2)
    lista[i], lista[j] = lista[j], lista_poligonos[i]

    
def No_Fit_Polygon():
    global lista_poligonos
    global nova_lista
    global nova_lista_completa
    global CordenadasProibidas
    tempo_anterior = 0
    colocou = False
    while len(lista_poligonos) > 0:
        ...
        while i < 345:
            ...
            while j < 385:
                ...
                poli = nova_lista[0]
                x_transformado, y_transformado = transformar_coordenadas_inversa(i, j, -200, 140, 205, -185, 345, 385)
    ...
    while len(lista_poligonos)>0:
        possiveis_posisoes = []
        for j in range(len(CordenadasProibidas)):
            if j == 2:
                n = 1
            else:
                n = 2
            for i in range(0,len(CordenadasProibidas[j]),2):
                if i < len(CordenadasProibidas[j]):
                    x1 = CordenadasProibidas[j][i][0]
                    y1 = CordenadasProibidas[j][i][1]
                for grau in graus:
                    for cor in adicionais:
                        x2,y2 = x1+cor[0],y1+cor[1]
                        #draw_piece(x2,y2,0)
                        cordenada_disponivel = True
                        poli = nova_lista[0]
                        pontos = []
...
def on_click2(x, y, grau,fecho_turtle, dados_turtle, botao):
    global CordenadasProibidas
    ...
    if botao.clicou(x,y):
        ...
        dados_turtle.clear()
        turtles[-1][0].clear()
        lista_poligonos.insert(0,turtles[-1][1])
        nova_lista.insert(0,turtles[-1][2])
        nova_lista_completa.insert(0,turtles[-1][3])
        ...
        dados_turtle.write(f"Area Desperdiçada = {round(((area_fecho-AreaPreenchida)/area_total)*100, 2)}", font=("Arial",16, "normal"))
        dados_turtle.penup()
        ...
        if len(lista_poligonos) > 0:
            ...
            if posição_disponivel:
                ...
                desenhar_fecho(vertices_fecho, fecho_turtle)
                turtle.tracer(1)
                turtles.append([p,lista_poligonos[0],nova_lista[0],nova_lista_completa[0]])
                lista_poligonos.pop(0)
                nova_lista.pop(0)
                nova_lista_completa.pop(0)
                return (((((area_fecho/area_total)**(1))*((AreaPreenchida/area_fecho)**(10)))))
...
melhor_area = 0
lista_atual = lista_poligonos.copy()
melhor_lista = lista_poligonos.copy()
for i in range(10000):
    ...
    area.greedy(-200,200)
    turtle.tracer(1)
    lista_poligonos = melhor_lista.copy()
    lista_atual = lista_poligonos.copy()
    Busca_Local(lista_poligonos)
    nova_lista, nova_lista_completa = tratar_lista(lista_poligonos) 
    No_Fit_Polygon()
    if (AreaPreenchida > melhor_area):
        melhor_area = AreaPreenchida
        melhor_lista = lista_atual.copy()
    print(i,(AreaPreenchida/(340*380))*100, len(CordenadasProibidas))
