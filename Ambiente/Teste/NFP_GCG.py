...
from CSP_GCG import ponto_dentro_poligono
...
def triangulate_shapely(points):
    """
    Triangula um polígono que pode ser côncavo.
    
    Parâmetros:
    points (list of tuples): Lista de coordenadas (x, y) dos pontos que formam o polígono.
    
    Retorna:
    list: Lista de polígonos (triângulos) que formam a triangulação.
    """
    # Se já for convexo, retorna o próprio polígono
    polygon = Polygon(points)
    if polygon.equals(polygon.convex_hull):
        return [points]

    # Prepara os dados no formato exigido pela biblioteca triangle
    vertices = np.array(points)
    segments = np.array([(i, (i + 1) % len(points)) for i in range(len(points))])
    
    # Define o dicionário de dados para triangulação
    poly = {
        'vertices': vertices,
        'segments': segments
    }
    ...
    # 'p': preserva a fronteira exata do polígono
    # 'q': qualidade da malha
    # 'a': área máxima (não usado aqui)
    triangulated = tr.triangulate(poly, 'p')
    
    # Extrai os triângulos
    triangles = []
    for triangle in triangulated['triangles']:
        triangle_points = [
            tuple(triangulated['vertices'][triangle[0]]),
            tuple(triangulated['vertices'][triangle[1]]),
            tuple(triangulated['vertices'][triangle[2]])
        ]
        triangles.append(triangle_points)
    
    # Filtra triângulos que estão dentro do polígono original
    valid_triangles = []
    for tri in triangles:
        tri_poly = Polygon(tri)
        if polygon.contains(tri_poly) or polygon.covers(tri_poly):
...
def nofitp(PolA,PolB):        
    ...
    for conv in convex_parts_B:
        ...
        for nfp in convex_parts_A:
            ...
            #for x1, y1 in pontos:
                #desenhar(conv, x1 * 50, y1 * 50, "green", draw=True)
...
if __name__ == "__main__":
    ...
    po1 = nova_lista_original[1]
    po2 = nova_lista_original[7]
    ...
    pol1 =[(0, 0), (1, 0), (1, 1), (2, 1), (2, 2), (1, 2), (1, 3), (0, 3), (0, 2), (-1, 2), (-1, 1), (0, 1)]
    pol1 = [(x-7,y) for x,y in pol1]
    pol = [(0, 0), (1, 0),(1, 3), (0, 3)]
    po = ajustar_poligono([(-1, 1),(2,1),(2,2),(-1,2)])
    ...
    "dimgray", "dodgerblue", "firebrick", "forestgreen", "fuchsia", "gold", "goldenrod", 
    "gray", "green", "greenyellow", "hotpink", "indianred", "indigo", "khaki", 
    "lawngreen", "lime", "limegreen", "magenta", "maroon", "mediumaquamarine", "mediumblue", 
    "mediumorchid", "mediumpurple", "mediumseagreen", "mediumslateblue", "mediumspringgreen", 
    "mediumturquoise", "mediumvioletred", "midnightblue", "navy", "olive", "olivedrab", 
    "orange", "orangered", "orchid", "palegoldenrod", "palegreen", "paleturquoise", 
    "palevioletred", "peru", "pink", "plum", "purple", "red", "rosybrown", "royalblue", 
    "saddlebrown", "salmon", "sandybrown", "sienna", "skyblue", "slateblue", "slategray", 
    "springgreen", "steelblue", "tan", "teal", "thistle", "tomato", "turquoise", 
    "violet", "yellow", "yellowgreen"
]

    
    desenhar(po2,0,0,random.sample(cores,1),True)
    #desenhar(pol1,0,0,random.sample(cores,1),True)


        
            
    #nfp4 = nofitp(pol1,copy.deepcopy(po1))
    #desenhar(nfp4, 0, 0, "green")


    nfp3 = nofitp(po2,copy.deepcopy(po1))
    nfp3 = remove_and_sort_vertices(nfp3)
    #desenhar(nfp3, 0, 0, "green")

    #nfp4 = list(combinar_poligonos([Polygon(nfp3), Polygon(nfp4)]).exterior.coords)
    ...
    #desenhar(nfp4_final, 0, 0, "red")
  

    # Desenhar várias cópias do polígono B nas posições calculadas pelo NFP
    for x1, y1 in nfp3:
    
        desenhar(po1, x1 * 50, y1 * 50, "green", draw=True)  # Mover B para as posições do NFP
        #desenhar(pol, x * 50, y * 50, "green", draw=True)
        #desenhar(po, x * 50, y * 50, "green", draw=True)
